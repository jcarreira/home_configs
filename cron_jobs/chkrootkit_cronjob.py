...
SUBJECT = f"chkrootkit Alert on {HOSTNAME}"
LOG_DIR = Path("/var/log/chkrootkit_check")
CURRENT_LOG = LOG_DIR / "chkrootkit_current.log"
PREVIOUS_LOG = LOG_DIR / "chkrootkit_previous.log"

SMTP_SERVER = "smtp.mail.yahoo.com"
SMTP_PORT = 587
SMTP_USERNAME = "jcmcarreira"
SMTP_PASSWORD = "ztjusagmmyavvuwk"
EMAIL_FROM = "jcmcarreira@yahoo.com"
EMAIL_TO = "jokarreira@gmail.com"
EMAIL_SUBJECT = "chkrootkit changed:"
...
def main():
    ...
    if CURRENT_LOG.exists():
        ...
        #f.write(f"Starting chkrootkit scan at {datetime.now().isoformat()}\n")

    run_command("chkrootkit", CURRENT_LOG)

    #with open(CURRENT_LOG, "a") as f:
    #    f.write(f"\nchkrootkit scan finished at {datetime.now().isoformat()}\n")
    ...
    if PREVIOUS_LOG.exists():
        try:
            ...
            if rest:
                ...
                email_body = (
                    "The chkrootkit scan results have changed since the last run.\n\n"
        ...
        email_body = "This is the first chkrootkit scan. The output is attached."
        send_email( f"Initial chkrootkit Run on {HOSTNAME}", email_body
        )
